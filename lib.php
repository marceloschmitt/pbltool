...
function print_project_tasks($courseid, $blockid, $groupid, $forum, $chatid, $teacher)
{
    ...
    if ($recordset) {
        ...
        $output .= '<tr>
                <th style="vertical-align:top; text-align:left;;white-space:nowrap;" class="header c0 firstcol"
                    scope="col">'.get_string('title','block_pbltool').'</th>
                <th style="vertical-align:top; text-align:center;;white-space:nowrap;" class="header c1"
                    scope="col">'.get_string('begin_date','block_pbltool').'</th>
                <th style="vertical-align:top; text-align:center;;white-space:nowrap;" class="header c2"
                    scope="col">'.get_string('finish_date','block_pbltool').'</th>
                <th style="vertical-align:top; text-align:center;;white-space:nowrap;" class="header c3"
                    scope="col">'.get_string('Progress','block_pbltool').'</th>
                <th style="vertical-align:top; text-align:center;;white-space:nowrap;" class="header c4"
                    scope="col">'.get_string('Status','block_pbltool').'</th>
                <th style="vertical-align:top; text-align:center;;white-space:nowrap;" class="header c5 lastcol"
                    scope="col">'.get_string('Actions','block_pbltool').'</th></tr>';
        $rowtype = '0';
        ...
        foreach ($recordset as $row) {
            ...
            $output .= '<a href=' . $CFG->wwwroot . '/blocks/pbltool/edit_task.php?courseid=' . $courseid . '&blockid=' .
                $blockid . '&groupid=' . $groupid . '&id='.$row->id.'>
                <img src="' . $OUTPUT->pix_url('t/edit') . '" alt="' . get_string('edit') . '"></a>&nbsp';
            if ($teacher){
                $output .= '<a href=' . $CFG->wwwroot . '/blocks/pbltool/delete_task.php?courseid=' . $courseid . '&blockid=' .
                $blockid . '&groupid=' . $groupid . '&id=' . $row->id.'>
                            <img src="'.$OUTPUT->pix_url('t/delete').'" alt="'. get_string('delete').'"></a>&nbsp;&nbsp;';
                switch($row->status) {
                    case 1: $output .= '<a href=' . $CFG->wwwroot . '/blocks/pbltool/task_change_status.php?courseid=' .
                                $courseid . '&blockid=' . $blockid . '&groupid='.$groupid . '&id='.$row->id . '&status=2>
                                <img src="images/ongoing.png" width="15"></a>&nbsp;&nbsp;';
                            $output .= '<a href=' . $CFG->wwwroot . '/blocks/pbltool/task_change_status.php?courseid=' .
                                $courseid .'&blockid=' . $blockid.'&groupid=' . $groupid.'&id=' . $row->id.'&status=3>
                                <img src="images/recycle.png" width="15"></a>&nbsp;&nbsp;';
                            break;
                    case 4: $output .= '<a href=' . $CFG->wwwroot . '/blocks/pbltool/task_change_status.php?courseid=' .
                                $courseid . '&blockid=' . $blockid . '&groupid=' . $groupid . '&id=' . $row->id . '&status=5>
                                <img src="images/checkmark.png" width="15"></a>&nbsp;&nbsp;';
                            $output .= '<a href=' . $CFG->wwwroot . '/blocks/pbltool/task_change_status.php?courseid=' .
                                $courseid . '&blockid=' . $blockid . '&groupid=' . $groupid . '&id=' . $row->id . '&status=6>
                ...
                switch ($row->status) {
                    case 0:
                    case 3: $output .= '<a href=' . $CFG->wwwroot . '/blocks/pbltool/delete_task.php?courseid=' . $courseid .
                                '&blockid=' . $blockid . '&groupid=' . $groupid . '&id=' . $row->id . '>
                                <img src="'.$OUTPUT->pix_url('t/delete').'" alt="'. get_string('delete').'"></a>&nbsp;&nbsp;';
                            $output .= '<a href=' . $CFG->wwwroot . '/blocks/pbltool/task_change_status.php?courseid=' .
                                $courseid . '&blockid=' . $blockid . '&groupid=' . $groupid.'&id=' . $row->id .'&status=1>
                                <img src="images/question.png" width="15"></a>&nbsp;&nbsp;';
                            break;
                    case 2:
                    case 6: $output .= '<a href='. $CFG->wwwroot . '/blocks/pbltool/task_change_status.php?courseid=' .
                                $courseid . '&blockid=' . $blockid . '&groupid=' . $groupid.'&id=' . $row->id.'&status=4>
...
function print_map($courseid,$blockid,$groupid,$teacher) {
    ...
    $graph->scale->actinfo->SetColTitles(array('Name', 'Start', 'Finish'), array(100));
